...
#    contigs=[]
    ...
    if s>0: 
        ...
        facing[n]="R"
    while len(q)>0 and ((not max_disp) or (abs(ptr)<max_disp)) :
        m=q.pop(0)
        coords[m]=ptr
        color[m]=1
...
def take_best_join_option(join_options,og,linked,edge_options,label="",threshold=0.0):
#    print join_options
    join_options.sort(reverse=True)
    print("join option:",join_options)
    if join_options[0][0]>threshold:
#        if len(join_options[0][2]) >=1 : print "#accept join",label, join_options[0], join_options
        print("accepting",label,join_options[0])
...
def link_test(og,a,b,gapsize=default_gapsize):
    coords = {}
    facing={}
    contig_list_a=traverse_and_layout(a,coords,facing,0,-1,og,200000)
    contig_list_b=traverse_and_layout(b,coords,facing,gapsize,+1,og,200000)
#    print "##x:setup done"
    sys.stdout.flush()
    ...
    np=0
    score=0.0
    for n1 in contig_list_a:
        for n2 in contig_list_b:
            if strand_check(n1,n2,coords,facing):
                distance = coords[n2] - coords[n1]
                if distance < 200000:
                    nlinks+=len(links.get((n1[:-2],n2[:-2]),[]))
    if nlinks==0: return -100
    for n1 in contig_list_a:
        for n2 in contig_list_b:
            if strand_check(n1,n2,coords,facing):
                distance = coords[n2] - coords[n1]
                if distance < 200000:
#                    print "\t".join(map(str,["#link_test debug:",n1,n2,distance,x]))
                    x=get_score(n1,n2,distance)
#                    print "#x:partial",n1,n2,x
                    score += x
#                    print "\t".join(map(str,["#link_test debug:",n1,n2,ll[n1[:-2]],ll[n2[:-2]],len(links.get((n1[:-2],n2[:-2]),[])),distance,x,score]))
                    np+=1
#                    sys.stdout.flush()
#    print "#score:",np,score
    return score

def nlinks_used(og,a,b,gapsize=default_gapsize):
    coords = {}
    facing={}
    contig_list_a=traverse_and_layout(a,coords,facing,0,-1,og,200000)
    contig_list_b=traverse_and_layout(b,coords,facing,gapsize,+1,og,200000)
#    traverse_and_layout(a,coords,facing,0,-1,og)
#    traverse_and_layout(b,coords,facing,gapsize,+1,og)
#    print "##x:setup done"
    sys.stdout.flush()
...
#    print "#intercalation test:",a,b,c
    ...
    coordinates2={}
    facing={}
    contig_list_a=traverse_and_layout(a,coordinates1,facing,0,-1,og,200000)
    coordinates2=dict(coordinates1)
    contig_list_c=traverse_and_layout(c,coordinates1,facing,default_gapsize,+1,og,200000+default_gapsize)
    contig_list_b=traverse_and_layout(b,coordinates2,facing,default_gapsize,+1,og,200000+default_gapsize)
    traverse_and_layout(c,coordinates2,facing,default_gapsize+max(coordinates2.values()),+1,og,200000+default_gapsize+max(coordinates2.values()))

    ###########Count the links anchoring the intercalating contig in place:
    nlinks=0
    for n1 in contig_list_a:
        for n2 in contig_list_b:
...
#                    nlinks+=1
    ...
    for n1 in contig_list_b:
        for n2 in contig_list_c:
            if strand_check(n1,n2,coordinates2,facing):
                distance = coordinates2[n2] - coordinates2[n1]
                if distance < 200000:
                    ...
                    nlinks+=len(links.get((n1[:-2],n2[:-2]),[]))
                   #nlinks+=1
    ########### Done counting the links
    if nlinks<2: return -100.0  # this is an optimization:  don't spend a ton of time calculating lots of intercalations supported by only a single chicago link.
...
if __name__=="__main__":
    ...
    parser.add_argument('-t','--threshold',default=12.0 ,  type=float)
...
#                        print "skip",cz[1:]
        ...
        for c1,c2 in link_pairs:
            if not c1<c2: continue
            if weights[c1,c2]>args.threshold:
                    ...
                    if (not c1 in linked) and (not c2 in linked): # and not c2 in nx.node_connected_component(og, c1):
                        ...
                        if (not c1 in linked) or (not c2 in linked):
                           ...
                           #  the scaffold being tested for insertion.
                            ...
                            take_best_join_option(join_options,og,linked,edge_options,"interc or end",args.threshold)
